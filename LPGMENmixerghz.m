function v = LPGMENmixerghz(n,d,k,phi,option)

%-------------------------------------------------------------------------%
%This function evaluates the linear program (LP) for a target pure
%n-partite high-dimensional GHZ state by using YALMIP.
...
for j = 1 : s
    ...
    if option == 0
        order = [set comp];
        [~,perm] = sort(order);
        rm = kpositiveghz(tau{j},k,n,d,set,perm);
        C = [C, rm >= 0];
    ...
    elseif option == 1
        ...
        order2 = [comp set];
        [~,perm2] = sort(order2);
        rm1 = kpositiveghz(tau{j},k,n,d,set,perm1);
        rm2 = kpositiveghz(tau{j},k,n,d,comp,perm2);
        C = [C, rm1 >= 0, rm2 >= 0];
    %Map on the biggest subset
    elseif option == 2
        order = [comp set];
        [~,perm] = sort(order);
        rm = kpositiveghz(tau{j},k,n,d,comp,perm);
        C = [C, rm >= 0];
    else
